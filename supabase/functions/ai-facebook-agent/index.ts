...
🔴 **CONFIRM THE ORDER** - Summarize: প্রোডাক্ট, দাম, নাম, ফোন, ঠিকানা। Ask "সব ঠিক আছে? অর্ডার কনফার্ম করব?"`;
  ...
  // ========== SELLING RULES (LIVE FROM DATABASE) ==========
  if (pageMemory.selling_rules) {
    prompt += `

## 💰 SELLING RULES (বিক্রয় নিয়ম):`;
    
    if (pageMemory.selling_rules.usePriceFromProduct) {
      prompt += `
- ✅ **শুধু প্রোডাক্ট ক্যাটালগের দাম ব্যবহার করুন** - নিজে দাম বানাবেন না`;
    }
    
    if (pageMemory.selling_rules.allowDiscount) {
      prompt += `
- ✅ **ডিসকাউন্ট দেওয়া যাবে** - সর্বোচ্চ ${pageMemory.selling_rules.maxDiscountPercent || 10}%`;
    } else {
      prompt += `
- ❌ **ডিসকাউন্ট দেওয়া যাবে না** - কাস্টমার চাইলে বলুন "দুঃখিত, এই প্রোডাক্টে ডিসকাউন্ট নেই"`;
    }
    
    if (!pageMemory.selling_rules.allowLowProfitSale) {
      prompt += `
- ❌ **কম লাভে বিক্রি করবেন না** - নির্ধারিত দামের নিচে যাবেন না`;
    }
  }

  // ========== AI BEHAVIOR RULES (LIVE FROM DATABASE) ==========
  if (pageMemory.ai_behavior_rules) {
    prompt += `

## 🤖 AI BEHAVIOR RULES (AI আচরণ):`;
    
    if (pageMemory.ai_behavior_rules.neverHallucinate) {
      prompt += `
- 🚨 **CRITICAL: কখনো দাম/তথ্য বানাবেন না** - প্রোডাক্ট ক্যাটালগে না থাকলে বলুন "এই প্রোডাক্টের তথ্য আমার কাছে নেই, একটু অপেক্ষা করুন"`;
    }
    
    if (pageMemory.ai_behavior_rules.askClarificationIfUnsure) {
      prompt += `
- ❓ **অনিশ্চিত হলে প্রশ্ন করুন** - কোন প্রোডাক্ট বুঝতে না পারলে জিজ্ঞেস করুন "কোন প্রোডাক্টের কথা বলছেন?"`;
    }
    
    if (pageMemory.ai_behavior_rules.askForClearerPhotoIfNeeded) {
      prompt += `
- 📸 **ছবি স্পষ্ট না হলে আবার চান** - বলুন "ছবিটা একটু ক্লিয়ার করে পাঠান"`;
    }
    
    if (pageMemory.ai_behavior_rules.confirmBeforeOrder) {
      prompt += `
- ✔️ **অর্ডার নেওয়ার আগে কনফার্ম করুন** - সব তথ্য রিপিট করে জিজ্ঞেস করুন "সব ঠিক আছে?"`;
    }
  }

  // ========== PAYMENT RULES (LIVE FROM DATABASE) ==========
  if (pageMemory.payment_rules) {
    prompt += `

## 💳 PAYMENT RULES (পেমেন্ট নিয়ম):`;
    
    if (pageMemory.payment_rules.codAvailable) {
      prompt += `
- ✅ **ক্যাশ অন ডেলিভারি (COD)** - "হ্যাঁ, ডেলিভারিতে পেমেন্ট করতে পারবেন"`;
    } else {
      prompt += `
- ❌ **ক্যাশ অন ডেলিভারি নেই** - অগ্রিম পেমেন্ট লাগবে`;
    }
    
    if (pageMemory.payment_rules.advanceRequiredAbove && pageMemory.payment_rules.advanceRequiredAbove > 0) {
      prompt += `
- 💵 **অগ্রিম পেমেন্ট:** ৳${pageMemory.payment_rules.advanceRequiredAbove} এর বেশি অর্ডারে ${pageMemory.payment_rules.advancePercentage || 50}% অগ্রিম লাগবে`;
    }
  }
